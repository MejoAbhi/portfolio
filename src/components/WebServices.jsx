...
const WebServices = ({ WebServicesModal,setWebServiceModal,modalType}) => {
    ...
    console.log("testtttttttttttttt",text)
        ...
        let web =[
            {
              ...
              "id": 0,
            //   "image":"E:\portfolio\src\assets\movieflix.jpg",
          ...
          <div className="sm:flex sm:items-start">
            <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
              <h3 className="text-base font-semibold leading-6 text-gray-900" id="modal-title">
                ...
                {modalType=="service"?"Web developpment":"Backend Development"}</h3>
              <div className="mt-2">
                <p className="text-lg text-black">{text}</p>
                <p className="text-sm text-gray-500">{desc}</p>
                {/* <img  className='rounded-s shadow w-fit' src={web.image}/> */}
              </div>
            </div>
          </div>
        </div>
        <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
          <button type="button" className="inline-flex w-full justify-center rounded-md bg-green-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-500 sm:ml-3 sm:w-auto" onClick={()=>modalType=="service"?handelNext(web):handelNext(backend)}>Next</button>

          <button  type="button" className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto" onClick={()=>setWebServiceModal(false)}>Cancel</button>
        </div>
      </div>
    </div>
  </div>
